...
export type ContentCalendarSSEEvent =
  ...
  | { type: 'calendar_complete'; session_id: string; report: string; quality_score: number; coherence_score: number; post_count: number; posts: Array<{ day: number; day_of_week: string; content_type: string; hook: string; body: string; cta: string; hashtags: string[]; posting_time: string; quality_score: number; word_count: number }>; themes?: ContentTheme[]; content_mix?: ContentMix }
