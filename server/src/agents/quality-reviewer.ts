...
1. Hiring Manager Impact (1-5): Would a skeptical hiring manager phone-screen this person in 30 seconds?
   ...
   - Evaluate: Does the summary immediately communicate the candidate's value proposition? Can you identify the candidate's unique positioning within 10 seconds? Are the first 3 bullets of each role the strongest? Is there a clear narrative arc, or does this read as disconnected job entries?
   - Example of a 5: A summary that names the candidate's domain, quantifies their scope, and connects directly to the target role's biggest challenge.
   - Example of a 2: A generic summary that could describe any executive at this level.
...
2. Requirement Coverage (0-100%): What percentage of JD must-haves are addressed?
   ...
   - Effective reframes demonstrate transferable capability with specific evidence. Example effective reframe: 'Led 200-person operations team across 3 regions' addresses 'global team management' even though the exact words don't match. Example weak reframe: 'Experience with team management' — too vague to convince a hiring manager.
...
4. Authenticity (0-100): Does this sound human-written or AI-generated?
   ...
   - Score 90+: Writing has distinctive voice, specific details only this candidate would know (company names, project names, specific metrics), varied sentence structures, natural language patterns. Note: if content is specific to scale, method, and outcome but omits company names (possibly due to NDA or confidentiality), treat it as authentic — specificity of experience matters more than named entities.
   - Score 50-69: Uniform 'Verb + metric + by doing X' pattern on every bullet, buzzwords like 'leveraged' and 'spearheaded' without specificity, every bullet is the same length and structure.
   - Score below 50: Could have been generated by pasting a JD into ChatGPT — no personal details, no authentic voice, interchangeable with any candidate.
...
5. Evidence Integrity (0-100): Are all claims supported by the evidence library?
   ...
   - Cross-reference EVERY quantified claim. 'Grew revenue 47%' must have evidence showing revenue growth of approximately 47%. If the evidence says 'significant revenue growth' but the resume says '47%', flag it — the specific number was fabricated.
   - Watch for: rounding inflation (evidence: 23% → resume: 'nearly 30%'), scope inflation (evidence: 'team of 8' → resume: 'organization of 50+'), attribution inflation (evidence: 'team achievement' → resume: implies solo accomplishment).
