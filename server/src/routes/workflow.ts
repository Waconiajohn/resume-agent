...
import {
  ...
  normalizePipelineActivityStatus,
  normalizeRuntimeMetrics,
  normalizeDraftPathDecision,
  normalizeSectionsBundleReview,
  normalizeBenchmarkEdit,
  normalizeWorkflowPreferences,
  persistPendingOrBufferedGateResponse,
  buildSkippedQuestionnaireSubmission,
  persistDraftNowRequest,
...
workflow.get('/:sessionId', rateLimitMiddleware(120, 60_000), async (c) => {
  ...
  const analyticsResult = questionnaireAnalytics(questionResponseRows ?? []);
  const historyResult = questionResponseHistory(questionResponseRows ?? []);
  const reuseSummaries = questionReuseSummaries(questionReuseSummaryRows ?? []);
  const reuseMetrics = questionReuseMetrics(reuseSummaries);
  ...
  return c.json({
    ...
    question_response_metrics: analyticsResult,
    question_response_history: historyResult,
    question_reuse_summaries: reuseSummaries,
    question_reuse_metrics: reuseMetrics,
    ...
    draft_readiness: normalizeDraftReadiness(draftReadinessRow, { normalizeCoverageOnlyReadiness }),
    pipeline_activity_status: normalizePipelineActivityStatus(pipelineActivityStatusRow),
    runtime_metrics: normalizeRuntimeMetrics(pipelineRuntimeMetricsRow),
    draft_path_decision: normalizeDraftPathDecision(draftPathDecisionRow, {
      normalizeDraftPathDecisionCompat,
      buildCoverageOnlyDraftPathDecisionMessage,
    }),
    sections_bundle_review: normalizeSectionsBundleReview(sectionsBundleRow),
    benchmark_edit: normalizeBenchmarkEdit(benchmarkEditRow),
    workflow_preferences: normalizeWorkflowPreferences(workflowPreferencesRow, pipelineStartRequestRow),
    replan_status: (() => {
