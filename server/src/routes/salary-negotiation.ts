...
salaryNegotiationRoutes.get('/reports/session/:sessionId', rateLimitMiddleware(30, 60_000), async (c) => {
  if (!FF_SALARY_NEGOTIATION) {
    return c.json({ error: 'Not found' }, 404);
  }
  ...
  if (error) {
    logger.error({ error: error.message, userId: user.id, sessionId: parsed.data }, 'GET /salary-negotiation/reports/session/:sessionId: query failed');
    return c.json({ error: 'Failed to fetch report' }, 500);
  }
  if (!data) {
    return c.json({ error: 'No reports found' }, 404);
