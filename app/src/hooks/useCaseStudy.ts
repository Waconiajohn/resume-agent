...
export type CaseStudyStatus = 'idle' | 'connecting' | 'running' | 'complete' | 'error';
...
export function useCaseStudy() {
  ...
  const connectSSE = useCallback(
    (sessionId: string) => {
      ...
      fetch(`${API_BASE}/case-study/${sessionId}/stream`, {
        ...
        .then(async (response) => {
          ...
          try {
            ...
            console.error('[useCaseStudy] SSE stream error:', err);
          }
  ...
  const startPipeline = useCallback(
    async (input: CaseStudyInput): Promise<boolean> => {
      ...
      try {
        const res = await fetch(`${API_BASE}/case-study/start`, {
          method: 'POST',
          headers: {
