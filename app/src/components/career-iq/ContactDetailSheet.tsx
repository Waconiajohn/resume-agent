      ...
      >
        <div className="flex items-start justify-between mb-5">
          <div>
            ...
            {contact.title && (
              <p className="text-[12px] text-white/40">{contact.title}</p>
            )}
            {contact.company && (
              <p className="text-[12px] text-white/30">{contact.company}</p>
            )}
          </div>
          <button
            type="button"
        ...
        <div className="space-y-2 mb-5">
          ...
          {contact.linkedin_url && (
            <div className="flex items-center gap-2 text-[12px] text-white/50">
              <Linkedin size={12} className="text-white/25 flex-shrink-0" />
              <a
        ...
        {contact.notes && (
          <div className="mb-5 rounded-lg border border-white/[0.06] bg-white/[0.02] px-4 py-3">
            <p className="text-[12px] text-white/50 leading-relaxed">{contact.notes}</p>
          </div>
        )}

        {/* Log touchpoint */}
        <div className="mb-5">
          <h3 className="text-[13px] font-medium text-white/65 mb-3">Log Interaction</h3>
          ...
          />
          {logError && <p className="mt-1.5 text-[11px] text-red-400">{logError}</p>}
        </div>

        {/* Touchpoint timeline */}
        {touchpoints.length > 0 && (
          <div>
            <h3 className="text-[13px] font-medium text-white/65 mb-3">Interaction History</h3>
              ...
              {touchpoints.map((tp) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {tp.notes && (
                      <p className="text-[11px] text-white/35 mt-0.5">{tp.notes}</p>
                    )}
                  </div>
                </div>
              ))}
        ...
        {touchpoints.length === 0 && (
          <p className="text-[12px] text-white/25 text-center py-3">No interactions logged yet.</p>
        )}
      </GlassCard>
    </div>
  );
