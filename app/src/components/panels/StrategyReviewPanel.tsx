    ...
    <GlassCard className={cn('p-4', colors.border)}>
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="flex items-center gap-1.5">
          {wasEdited && (
            <span className={cn('rounded-md border px-2.5 py-1 text-[12px] uppercase tracking-[0.10em]', colors.badge)}>
              Edited
            </span>
          )}
          {canEdit && !wasEdited && (
            <span className="rounded-md border border-[var(--line-soft)] bg-[var(--accent-muted)] px-2.5 py-1 text-[12px] uppercase tracking-[0.10em] text-[var(--text-soft)]">
              Editable
            </span>
          )}
        </div>
        ...
        >
          <div className="flex items-start gap-2 mt-1">
            <p className={cn(
              ...
              wasEdited ? colors.icon : 'text-[var(--text-strong)]',
            )}>
              {currentValue}
            </p>
            {canEdit && (
              <Pencil className="h-3.5 w-3.5 shrink-0 mt-0.5 text-white/0 group-hover:text-[var(--text-soft)] transition-colors" />
            )}
          </div>
        </button>
      )}
      ...
      <div data-panel-scroll className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        <GlassCard className="p-3">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[12px] font-semibold uppercase tracking-wider text-[var(--text-soft)]">Negotiation Approach</span>
          </div>
          <p className="text-sm text-[var(--text-strong)] capitalize">{approach}</p>
        </GlassCard>
          ...
          <div className="grid grid-cols-2 gap-2">
            {market_p50 != null && (
              <div className="rounded-lg border border-[var(--line-soft)] bg-[var(--accent-muted)] px-3 py-2">
                <p className="text-[12px] text-[var(--text-soft)] mb-0.5">Market Median (P50)</p>
                <p className="text-sm font-medium text-[var(--text-muted)]">${market_p50.toLocaleString()}</p>
              </div>
            )}
            {market_p75 != null && (
              <div className="rounded-lg border border-[var(--line-soft)] bg-[var(--accent-muted)] px-3 py-2">
                <p className="text-[12px] text-[var(--text-soft)] mb-0.5">Market P75</p>
                <p className="text-sm font-medium text-[var(--text-muted)]">${market_p75.toLocaleString()}</p>
              </div>
            )}
          </div>
        )}
