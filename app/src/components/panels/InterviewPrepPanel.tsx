...
function QuestionItem({ q }: { q: InterviewQuestion }) {
  ...
  return (
    <div
      className="rounded-xl border border-white/[0.12] bg-white/[0.05] overflow-hidden transition-all duration-200"
    >
      <button
        onClick={() => setExpanded(!expanded)}
        className="flex w-full items-start gap-2 p-3 text-left hover:bg-white/[0.06] transition-colors"
      >
        {expanded ? (
          <ChevronDown className="mt-0.5 h-3.5 w-3.5 shrink-0 text-white/50" />
        ) : (
      ...
      {expanded && (
        <div className="border-t border-white/[0.12] px-3 pb-3 pt-2 space-y-3">
          {/* Why they ask */}
          <div className="flex items-start gap-2">
            <HelpCircle className="mt-0.5 h-3 w-3 shrink-0 text-purple-400" />
            <p className="text-xs text-white/70 italic">{q.why_asked}</p>
          </div>
    ...
    <div className="flex h-full flex-col">
      <div className="border-b border-white/[0.12] px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-white/85">Interview Prep</span>
        </div>
      </div>
