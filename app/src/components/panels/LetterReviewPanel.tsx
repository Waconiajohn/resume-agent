...
import { FileText, CheckCircle2, MessageSquare, Pencil, ArrowRight } from 'lucide-react';
import { GlassCard } from '../GlassCard';
import { GlassButton } from '../GlassButton';
import { cn } from '@/lib/utils';
import type { LetterReviewData } from '@/types/panels';

interface LetterReviewPanelProps {
  data: LetterReviewData;
  onPipelineRespond?: (gate: string, response: unknown) => void;
}

type Mode = 'review' | 'request_changes' | 'edit_directly';

export function LetterReviewPanel({ data, onPipelineRespond }: LetterReviewPanelProps) {
  const { letter, quality_score } = data;

  const [mode, setMode] = useState<Mode>('review');
  const [feedback, setFeedback] = useState('');
  const [editedContent, setEditedContent] = useState(letter);

  const handleApprove = useCallback(() => {
    onPipelineRespond?.('letter_review', true);
  }, [onPipelineRespond]);

  const handleSubmitFeedback = useCallback(() => {
    if (!feedback.trim()) return;
    onPipelineRespond?.('letter_review', { approved: false, feedback: feedback.trim() });
  }, [onPipelineRespond, feedback]);

  const handleSubmitDirectEdit = useCallback(() => {
    if (!editedContent.trim()) return;
    onPipelineRespond?.('letter_review', { approved: false, edited_content: editedContent.trim() });
      ...
      <div className="border-b border-white/[0.12] px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-white/85">Cover Letter Review</span>
          </div>
          {quality_score != null && (
      ...
      <div data-panel-scroll className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        <GlassCard className="p-3">
          <p className="text-xs text-white/65 leading-relaxed">
            Review your cover letter below. Approve it as-is, request specific changes, or edit it directly before finalizing.
          </p>
        </GlassCard>
        ...
        {mode === 'edit_directly' ? (
          ...
          <GlassCard className="p-4">
            <p className="whitespace-pre-wrap text-xs text-white/85 leading-relaxed">{letter}</p>
          </GlassCard>
        )}
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-2">
                <GlassButton
                  ...
                  aria-label="Request changes to the cover letter"
                >
                ...
                <GlassButton
                  variant="ghost"
                  className="w-full"
                  onClick={() => { setEditedContent(letter); setMode('edit_directly'); }}
                  aria-label="Edit cover letter directly"
                >
