...
export default function ProfileSetupPage() {
  ...
  const handleRetryMasterResume = useCallback(async () => {
    ...
    if (result) {
      if (result.master_resume_created === false) {
        ...
        trackProductEvent('profile_setup_retry_failed', {
          ...
          message: 'Achievement proof creation still needs another retry.',
        });
        dispatch({
          type: 'SET_ERROR',
          error: 'We saved your Career Vault again, but your achievement proof still needs another retry.',
        });
        return;
      }
    ...
    trackProductEvent('profile_setup_retry_failed', {
      ...
      message: hookErrorRef.current ?? 'Could not create your achievement proof yet. Please try again.',
    });
    dispatch({
      type: 'SET_ERROR',
      error: hookErrorRef.current ?? 'Could not create your achievement proof yet. Please try again.',
    });
