...
### Changes Made
- `server/src/routes/application-timeline.ts` — new `GET /timeline/all` handler appended above the per-pursuit `GET /:applicationId/timeline`. Filters non-terminal applications (stage NOT IN `offer | closed_won | closed_lost`), capped at 50 rows ordered by recent `updated_at`. Then issues 9 bulk queries in parallel — `coach_sessions`, `cover_letter_reports`, `interview_prep_reports`, `thank_you_note_reports`, `follow_up_email_reports`, `networking_messages`, `application_events`, `company_directory`, `referral_bonus_programs` — using `.in('job_application_id', appIds)` to fetch in batches. Per-app payloads assembled in JS from the bulk results via `Map`-keyed lookups. `archived_at IS NULL` filter excludes archived rows from Today.
- `app/src/lib/timeline/today.ts` — new aggregator. Pure function `aggregateTodaySignals(payloads, options)` returning `{ tierA, tierB, tierC }`. Tier A is hand-rolled (overdue thank-you = interview > 24h ago + no thank-you sent; today/tomorrow interviews = scheduled_date date-bucket match; imminent prep = scheduled_date within 3 days + no prep brief). Tier B/C reuse `computeTimelineRules`, suppressing N5/N6 entries (already covered by tier A's tighter thresholds).
- `app/src/hooks/useTodayTimeline.ts` — new fetch hook calling `/api/job-applications/timeline/all`. Returns `{ pursuits, aggregation, loading, error, totalCount, refresh }`. Composes `aggregateTodaySignals` with the bulk payload via `useMemo`.
- `app/src/components/applications/TodayView.tsx` — new component. Three regions stacked (Time-sensitive / Get unblocked / Waiting), each rendering a list of `TodayItemCard`s. Each card shows company + role (deep-links to `/workspace/application/:id/overview`), action label from the rule engine, and a small days indicator (`today`, `tomorrow`, `1 day ago`, `in 3 days`). Tier-A cards get the `AlertCircle` urgency icon; tier-B get `Sparkles`; tier-C get `Hourglass`. Empty state nudges toward `/workspace/job-search`. Skeleton loader matches the three-region shape.
- `app/src/components/career-iq/ApplicationsListScreen.tsx` — added Today/Pipeline tab switcher above the existing Active/Archived toggle. URL persistence via `?view=pipeline` (Today is default — no param). Today tab renders `<TodayView />`; Pipeline tab renders the existing kanban content unchanged. Active/Archived sub-toggle hides on Today tab (it doesn't apply).
- `app/src/__tests__/lib/timeline-today.test.ts` — 13 unit tests for the aggregator: tier-A overdue thank-you fires/suppresses, today's/tomorrow's interview bucketing, imminent prep window, prep suppression once brief exists, overdue thank-yous always lead within tier A, tier B picks up N1, N5/N6 suppressed in tier B, tier C surfaces T1, cross-pursuit fan-out, all-quiet empty state.
- `app/src/__tests__/applications/TodayView.test.tsx` — 7 component tests with hook stubbed: skeleton path, empty state, tier-A region rendering, tier-B region rendering, tier-C region rendering, click-to-pursuit-overview routing, click-to-action-target routing.
- `server/src/__tests__/application-timeline-bulk.test.ts` — 5 server tests with the supabase-js chain mocked: terminal-stage filtering, empty-pursuits response, 50-row cap on the apps query, 500 on apps-query DB error, payload shape conformance.

### Decisions Made
- **Server returns raw payloads, client computes rules.** Same shape as the per-pursuit endpoint, just wrapped in `{ pursuits: [...] }`. The rule engine remains a single TypeScript source on the client; no porting to the server. Wire size for 50 pursuits is ~50–100 KB — small enough not to matter, big enough to amortize the cost of one round-trip.
- **Bulk queries instead of N×9 round-trips.** Each artifact table is queried once with `.in('job_application_id', appIds)` so the total query count is constant (~9) regardless of how many pursuits the user has. The per-app assembly happens in JS via `Map` lookups.
- **Tier A is hand-rolled, not derived from N5/N6.** N5 (interview prep) fires for interviews scheduled within ~7 days; N6 (thank-you) fires within 48hrs. Today's tier A wants tighter / different windows: today vs tomorrow vs imminent (3-day) for interviews; OVERDUE (>24h) for thank-yous. Aggregator emits these directly and suppresses N5/N6 from the tier B/C rule reuse to avoid duplicates.
- **Tier C reuses Their-turn rules verbatim.** Same engine output, mapped into the cross-pursuit format. Lets users see "Acme — applied 5 days ago, no response" in the daily view alongside "Beta — quiet in screening for 30 days."
- **Today is the default tab; Pipeline is the kanban.** The kanban answers "what shape is my pipeline." Today answers "what should I do right now." Most users open this surface to find an action, not to study the pipeline shape — Today is the better default.
- **URL persistence via `?view=` (Pipeline only) — Today is the empty default.** Conventionally cleaner than `?view=today`. Pipeline gets the explicit param; Today is the unmarked-canonical state.
- **50-pursuit cap.** Spec said it; the practical reasoning is that anyone tracking 50+ active pursuits has an organizational problem that the Today view can't fix — it would just be a wall of cards. Cap is server-side; the cap doesn't change the rule engine output for any single pursuit.
- **Non-terminal definition for Today differs from the existing TERMINAL_STAGES in ApplicationsListScreen.** The kanban's TERMINAL_STAGES is `['closed_won', 'closed_lost']`. Today's exclusion adds `'offer'` because by then the user is in negotiation mode (N7 fires, but inside the per-pursuit workspace; cross-pursuit Today is for in-flight pursuits). Different question, different definition of "active".
- **Active/Archived sub-toggle hides on Today.** Today is by definition active — it can't surface signals for archived pursuits because they're, well, archived.
- **Empty state nudges toward `/workspace/job-search`.** Spec said "good time for prospecting." We deep-link to the job search route. If that surface doesn't exist for some user routing, the link still resolves to a 404 they can recover from — not a crash.
- **Click-target routing.** Card body (company/role) routes to the pursuit's `/overview` so the user can see full context. Action button routes to the rule engine's `target` so the user can act immediately. Two affordances on one card.

### Known Issues
- None introduced. Server tests: **2723 / 0 failing** (+5 new). App tests: **2040 passing / 10 failing / 10 skipped** — matches the pre-Phase-5 baseline of 2020 passing exactly, +20 new tests (13 aggregator + 7 component).
- The `?view=` URL state is `replace`-mode (no history entries on toggle). Browser back from Today doesn't go to Pipeline — it leaves the screen entirely. This matches how the existing Active/Archived toggle behaves; consistent across the screen.

### Next Steps
- Pursuit-timeline work is complete (Phases 1–5 shipped). Remaining open work tracked in MEMORY.md (mobile/a11y pass — Sprint F; resume parser regression; production push priorities; LinkedIn data persistence; Chrome extension install path).
