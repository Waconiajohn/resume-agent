...
## ADR-049: Profile Setup as Master Resume Genesis Event
**Date:** 2026-04-06
**Status:** accepted
**Context:** The platform has three separate data stores for career information that do not communicate. The CareerIQ profile (`user_platform_context`, `context_type='career_iq_profile'`) stores narrative positioning — career thread, capabilities, signature story, Why Me — as free-text not anchored to specific companies or positions. The master resume (`master_resumes` table) stores structured experience with company, title, dates, bullets, and evidence items. Raw resume text is pasted fresh at the start of each pipeline session. The CareerIQ profile setup already conducts a targeted 8-question interview that surfaces rich career stories, but interview answers live only in an in-memory session with a 2-hour TTL and evaporate after synthesis. The existing questions already carry a `references_resume_element` field linking each question to a specific resume element — the company-mapping data exists but is discarded. The master resume builds incrementally through per-session promotion, so the first resume pipeline run has no enriched master resume to draw from. This creates three compounding problems: evidence in the CareerIQ profile cannot be placed under specific job headings when writing resumes; the richest career material (interview answers) is ephemeral; and the master resume and CareerIQ profile are parallel tracks with no bridge between them.
**Decision:** Profile setup becomes the genesis event that creates both the CareerIQ profile (identity and positioning layer) and the initial master resume (structured content layer) from the same source material in a single flow. The intake agent will extract structured experience entries — company, title, dates — alongside its existing analysis. Interview answers will be mapped to specific experience entries using the existing `references_resume_element` field on each question. The `/complete` endpoint will atomically create both the CareerIQ profile and the initial master resume, with enriched evidence from interview answers anchored to the correct companies. The "Why Me" synthesis will be strengthened to pass a 3-5 second test — leading with the most powerful, differentiated statement about the candidate, suitable for seeding the professional summary section. Raw interview answers will be persisted to `user_platform_context` as a separate context type so the source material remains available for future resume sessions and other downstream agents.
**Reasoning:** Profile setup already receives resume text and conducts a targeted 8-question interview — it holds all the raw material needed to produce both outputs simultaneously. The `references_resume_element` field already exists on interview questions, meaning the company-mapping contract is established but unused; no new data needs to be collected. Building the master resume at profile setup means the first resume pipeline run has enriched structured data from day one rather than starting from a blank slate. The two outputs serve distinct downstream needs: the profile guides agent tone, positioning, and narrative context across all 33 products, while the master resume provides structured, pageable content for resume generation and exports. Persisting raw interview answers prevents the most valuable career material from being ephemeral and makes it reusable by the LinkedIn optimizer, executive bio, networking outreach, and any other agent that needs authentic career evidence. A "Why Me" that leads with a candidate's strongest differentiator becomes the natural seed for professional summary sections, reducing the Craftsman's dependence on inferring positioning from raw experience entries alone.
**Consequences:** The intake agent must produce a structured experience extraction alongside its current analysis output. The synthesizer needs a company-mapping step that anchors interview evidence to experience entries before creating the master resume. The `/complete` endpoint must grow to create both the CareerIQ profile and the master resume atomically — if either write fails, neither should commit. The "Why Me" prompt must be redesigned to lead with the most powerful differentiator rather than generic positioning language. Raw interview answers must be persisted to `user_platform_context` as a new context type (e.g., `profile_interview_answers`). Resume V2's Candidate Intelligence agent can optionally read from the master resume instead of re-parsing raw text, reducing duplicate extraction work across sessions. Existing profile-setup tests need updating to assert on the expanded output including the master resume record.
- future work should prioritize upstream context quality and shared contracts before room-specific rescue logic

## ADR-050: Provider-Neutral Auth Boundary for Outplacement
**Date:** 2026-04-30
**Status:** accepted

**Context:** The consumer app already runs on Supabase Auth, while the outplacement motion introduces employer organizations, HR admins, employee seats, and future enterprise SSO/SCIM pressure. Running a second auth provider directly through product routes would split authorization logic, duplicate user identity handling, and force a larger rewrite once B2B contracts require SSO.

**Decision:** Keep Supabase Auth as the live session provider and introduce a provider-neutral bridge before adding any Clerk or WorkOS integration. `platform_auth_identities` maps provider subjects to the canonical platform user. `b2b_organization_members` maps users and invited emails to organization roles and seat links. Server code authorizes B2B admin access against organization membership, with `b2b_organizations.admin_email` retained only as a compatibility fallback and primary-contact field.

**Reasoning:** Most product tables are correctly scoped to `auth.users(id)` today. Replacing that foundation during launch hardening would be high-risk. A bridge gives us the important part now: every auth provider must resolve to the same app user and organization membership model before routes see it. That keeps consumer auth and outplacement auth from becoming separate systems.

**Consequences:** Future Clerk or WorkOS work must be implemented as an adapter that writes/updates these bridge tables first. SCIM, if added, provisions memberships and seat state; it does not introduce separate app roles. Product routes should never authorize directly against a Clerk ID, WorkOS ID, or SAML subject.
